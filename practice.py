...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    if numbers == []:
        ...
        smallest_int_tracker = numbers[0]
        for number in numbers:
            if number < smallest_int_tracker:
                smallest_int_tracker = number

    return smallest_int_tracker
...
def largest_int(numbers):
    ...
    **DO NOT USE** the built-in function `max()`!
    ...
    if numbers == []:
        ...
        largest_int_tracker = numbers[0]
        for number in numbers:
            if number > largest_int_tracker:
                largest_int_tracker = number

    return largest_int_tracker
...
def average(numbers):
    ...
    return float(sum_nums) / len(numbers)
...
def join_strings_with_comma(words):
    ...
    if len(words) > 1:
        for word in range(1, len(words)):
            joined_string_with_commas += ", " + words[word]
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    # if len is even, midpoint is last switch
    # if len is odd, midpoint truncated to last necessary switch
    midpoint = last_index / 2

    for item in range(0, midpoint):
        temp = items[last_index]
        items[last_index] = items[item]
        items[item] = temp
        last_index -= 1
